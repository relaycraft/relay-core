...
async fn ca_install_guide(_ctx: &Arc<ProbeContext>) -> Result<Vec<ResourceContents>, ToolError> {
    let ca = CaPaths::resolve(None, None).unwrap_or_else(|_| CaPaths::default_from_data_dir());
    let guide = format!(
        r#"# Install RelayCore CA Certificate
...
Copy and paste the install command for your OS.
...
## Linux (Debian/Ubuntu)
```bash
sudo cp "{}" /usr/local/share/ca-certificates/relay-core.crt && sudo update-ca-certificates
```

## Linux (Fedora/RHEL)
```bash
sudo cp "{}" /etc/pki/ca-trust/source/anchors/relay-core.crt && sudo update-ca-trust
```

## Windows
1. Press `Win + R`, type `certmgr.msc`, press Enter
2. Right-click "Trusted Root Certification Authorities" → All Tasks → Import
3. Select the file above (change file filter to "All Files")
