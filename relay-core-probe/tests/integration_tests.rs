...
fn new_server() -> relay_core_probe::ProbeServer {
    ...
    ProbeServer::new(
        ProbeConfig { transport: ProbeTransport::Stdio },
        Arc::new(tokio::runtime::Runtime::new().unwrap().block_on(CoreState::new(None))),
    )
}
...
fn parse_tool_output(content: &[rmcp::model::Content]) -> Value {
    ...
    let text = serialized["text"].as_str().expect("content should have text field");
...
fn extract_text(content: &[rmcp::model::Content]) -> String {
    serde_json::to_value(&content[0]).unwrap()["text"].as_str().unwrap().to_string()
}
...
fn all_15_tool_schemas_registered() {
    ...
    for expected in [
        "search_flows", "get_flow", "get_metrics", "replay_flow", "export_har",
        "set_intercept", "get_pending_intercepts", "resume_flow",
        "set_rule", "delete_rule", "mock_url",
        "get_policy", "update_policy", "patch_policy", "set_script",
    ] {
...
fn tool_schemas_have_descriptions() {
    for tool in tools::tool_list() {
        assert!(tool.description.as_deref().is_some_and(|d| !d.is_empty()), "tool {} has no description", tool.name);
    }
}
...
async fn search_flows_with_filters() {
    let ctx = new_ctx().await;
    let result = tools::search_flows(&ctx, json!({"host": "x", "method": "GET", "limit": 5})).await.unwrap();
    assert!(parse_tool_output(&result).is_array());
}
...
async fn get_flow_nonexistent_errors() {
    let ctx = new_ctx().await;
    assert!(tools::get_flow(&ctx, json!({"id": "00000000-0000-0000-0000-000000000000"})).await.is_err());
}
...
async fn get_metrics_has_expected_keys() {
    ...
    for key in ["flows_total", "intercepts_pending", "rule_exec_errors", "audit_events_total"] {
...
async fn update_policy_roundtrip() {
    let ctx = new_ctx().await;
    let policy = json!({"transparent_enabled": true, "redaction": {"enabled": true, "redact_bodies": true}});
    tools::update_policy(&ctx, json!({"policy": policy})).await.unwrap();
...
async fn patch_policy_toggles_redaction() {
    let ctx = new_ctx().await;
    tools::patch_policy(&ctx, json!({"patch": {"redaction": {"enabled": true}}})).await.unwrap();
    let json = parse_tool_output(&tools::get_policy(&ctx).await.unwrap());
    assert_eq!(json["redaction"]["enabled"], true);
}
...
async fn set_and_delete_rule() {
    let ctx = new_ctx().await;
    let rule = Rule {
        id: "probe-test-rule".to_string(), name: "Probe Test".to_string(),
        active: true, stage: RuleStage::RequestHeaders, priority: 10,
        termination: RuleTermination::Continue, filter: Filter::All,
        actions: vec![Action::AddRequestHeader { name: "x-test".to_string(), value: "1".to_string() }],
        constraints: None,
    };
    let result = tools::set_rule(&ctx, json!({"rule": serde_json::to_value(&rule).unwrap()})).await.unwrap();
    assert!(extract_text(&result).contains("set successfully"));

    let result = tools::delete_rule(&ctx, json!({"id": "probe-test-rule"})).await.unwrap();
...
async fn delete_nonexistent_rule_errors() {
    let ctx = new_ctx().await;
    assert!(tools::delete_rule(&ctx, json!({"id": "never-exists"})).await.is_err());
}
...
async fn set_intercept_creates_breakpoint() {
    let ctx = new_ctx().await;
    let result = tools::set_intercept(&ctx, json!({"url_pattern": "example.com/api", "phase": "request"})).await.unwrap();
...
async fn proxy_status_has_consistent_shape() {
    let ctx = new_ctx().await;
    let contents = resources::read_resource(&ctx, "proxy://status").await.unwrap();
    let text = parse_resource_output(&contents);
...
async fn audit_resource_events_is_array() {
    let ctx = new_ctx().await;
    let contents = resources::read_resource(&ctx, "audit://recent").await.unwrap();
    let text = parse_resource_output(&contents);
    let json: Value = serde_json::from_str(&text).expect("valid JSON");
...
async fn ca_install_guide_has_os_sections() {
    let ctx = new_ctx().await;
    let contents = resources::read_resource(&ctx, "ca://install").await.unwrap();
    let text = parse_resource_output(&contents);
...
async fn dispatch_unknown_tool_not_found() {
    let ctx = new_ctx().await;
    let err = tools::dispatch(&ctx, "does_not_exist", json!({})).await.unwrap_err();
...
async fn export_har_missing_flow_errors() {
    let ctx = new_ctx().await;
    assert!(tools::export_har(&ctx, json!({"id": "00000000-0000-0000-0000-000000000000"})).await.is_err());
}
