...
#[command(
    name = "relay-core",
    // argv[0] is the native file (`relay-core-cli` / `relay-core-cli.exe`). Help must show the
    // command npm installs, which is `relay-core`.
    bin_name = "relay-core",
    display_name = "relay-core",
    version,
...
Examples:
  ...
  relay-core analyze --file export.har --format har  Analyze HAR export
  relay-core scripts init                        Scaffold a new script project
  relay-core scripts build                       Bundle script with esbuild
