...
## 跨版本比较前必须核对 environment 字段

Baseline JSON 里的 `environment.os` 必须一致，否则数字不可比。v0.12.0 就是实例：吞吐比
v0.11.0 低 16.9%（40,199.6 vs 48,380.2 req/s），而在代码里怎么找都找不到原因——**因为原因不在代码里**：

| | v0.11.0 | v0.12.0 |
|---|---|---|
| `environment.os` | macOS 26.5.2 | **macOS 27.0** |
| 测量时间（本地） | 09-14 03:09 | 09-16 03:53 |

`/var/log/install.log` 记录 macOS 27.0 安装于 **09-15 22:59**，即两次测量之间。
P99 反而更好（0.74 vs 1.07ms）也不符合"机器变慢"，而是换了一个系统。

被数字排除的代码嫌疑（各测一次即可排除）：

- pump 里的捕获排除检查：开启 40,153 / 39,927，**绕过 40,127** ⇒ 不花可测量成本；
- 每请求的 `server_host` 格式化：**去掉 40,250**，保留 40,199 ⇒ 同样如此。

**结论**：跨 OS 升级的 baseline 不可直接比较。发布说明里出现吞吐回退时，先核对 `environment`，
再花时间读代码。
