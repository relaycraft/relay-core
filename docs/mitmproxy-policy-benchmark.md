...
| 解码编码集 | gzip / deflate / br / zstd | gzip / deflate / br / zstd（**已对齐**） |
| 写回行为 | 自动重编码，`content-length` 同步 | 自动重编码，`content-length` 同步（**已对齐**） |
| header 与字节一致性 | 始终一致 | 始终一致 |
...
| 项 | 建议 | 理由 |
|---|---|---|
| 补 `br` / `zstd` 解码重编码 | **已完成** | 编解码能力已与 mitmproxy 对齐 |
