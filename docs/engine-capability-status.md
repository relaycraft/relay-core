...
> **§4 关系字段进展**（本次）：`NetworkInfo.server_host` 已记录正代代理请求的**目标**
> （`host:port`，端口按 scheme 补全），UI 因此不再显示 `unknown`。**仍未做**：
> `connection_id` / `parent_flow_id` / `stream_id`，以及对端**已解析地址**（正向代理不建连，
> 无从得知；透明捕获路径才有）。
...
| §6 HTTP/2 | **Working** | 明文 h2c **双向可用**（入站含 CONNECT 隧道内 + 出站含 h2c-only 上游）；TLS 后的 H2 亦有线路断言。两种 h2c 形态 mitmproxy 都不支持 | trailers 现已**记录进 Flow**；仍无 gRPC **消息级解析**；无 RST_STREAM/GOAWAY |
