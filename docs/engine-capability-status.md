...
| §6 HTTP/2 | **Working** | 明文 h2c **双向可用**（入站含 CONNECT 隧道内 + 出站含 h2c-only 上游）；TLS 后的 H2 亦有线路断言。两种 h2c 形态 mitmproxy 都不支持 | trailers 能透传但**未记入 Flow**（捕获里看不到 `grpc-status`）；无 gRPC 消息级解析；无 RST_STREAM/GOAWAY |
