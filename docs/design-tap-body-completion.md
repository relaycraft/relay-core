...
- **Status**: Design / **方案 A 已被实验证伪**（见 §6bis），待重新设计
- **Date**: 2026-09-16
...
## 6bis. 实验结论（2026-09-16，已跑完）

两个实验都做了，**方案 A 被自己的实验证伪**——这正是先做实验的意义。

### 实验 1：回调里只做 O(1)「移动 + 发送」

`Drop` 里只做 `take_retained()` + `send`（接收端丢弃）：`test_h1_concurrent_connections`
**1.28s 通过、无挂起**。⇒ 假设"回调只做 O(1) 就不会丢唤醒"**成立**。

### 实验 2：接上真正的消费端

给消费端接上 framing 后逐步 bisect：

| 变体 | 结果 |
|---|---|
| 消费端**只排空**通道 | ✅ 通过（1.37s） |
| 消费端**做 framing、不发更新** | ✅ 通过（1.50s） |
| 消费端**做 framing + 发出 `HttpBody` 更新** | ❌ **挂起** |

**结论与设计假设相反**：挂起与"回调里做多少工作"无关，也与 framing 计算无关——
**是「在 drop 时刻额外发出的那个更新」本身**。方案 A（把工作挪出回调）**不解决问题**。

### 这对设计意味着什么

1. **方案 A 作废**（连同"工作量在回调里"这条解释一起）。
2. 真正可疑的是**更新到达 runtime 的时机/路径**：drop 时刻发出的 `FlowUpdate::HttpBody`
   会在**最终 `Full` 快照之后**（甚至交换已结束之后）到达，而 runtime 的
   `update_http_body` 会**让 flow store 重新持久化该 flow**。下一步应当先查这条路径
   （store actor 是否被这条更新拖住），而不是继续在 tap 里换触发点。
3. `文档 §2 的"约束 2"（回调里做重活有风险）已被实验推翻`，保留原文以示更正过程，但**不要**据此设计。
4. 若查证后确认这条路径不可用，则回到**方案 C**（自建连接驱动，独占"响应写完"时刻）。

### 已知未被实验排除的替代方向

- 让 drop 时刻**不发送**新更新，而是**把已观测字节并入一个稍后的、已在途的更新**（例如由
  runtime 在收尾时统一取一次）；
- 让 body 的字节**不随 body 生命周期走**，而是由代理侧在请求开始时登记、在收尾时读取
  （即"字节归属代理，而非 body"），这样完成时刻由我们决定，且没有任何回调参与。
