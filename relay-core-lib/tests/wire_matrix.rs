            ...
            tokio::spawn(async move {
                let service = service_fn(move |_req: hyper::Request<hyper::body::Incoming>| {
                    ...
                    async move {
                        ...
                        let mut response = hyper::Response::new(make_body());
...
/// A captured gRPC call must be legible as messages, not as an opaque body.
///
/// The framing is what makes a record usable: one length-prefixed message is a unary call, several are
/// a stream. As base64 alone the capture only proves that something was sent. The upstream declares
/// `application/grpc+proto` because that header is what identifies the framing — a gRPC response
/// without it is not gRPC as far as the capture is concerned.
#[tokio::test]
async fn wire_matrix_grpc_body_is_captured_as_messages() {
    use bytes::Bytes;
    use hyper::body::Frame;

    /// One length-prefixed message of five bytes, then end of stream.
    ...
    // Bodies are reported as incremental updates, which is how a streamed body reaches a consumer.
    let deadline = tokio::time::Instant::now() + std::time::Duration::from_secs(3);
