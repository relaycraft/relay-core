...
use relay_core_lib::rule::engine::state::{InMemoryRuleStateStore, RuleStateStore};
...
impl RuleStoreActor {
    ...
    pub async fn run(mut self) {
            ...
            && let Ok(loaded_rules) = store.load_rules().await
        {
            ...
            if !rules.is_empty() {
                self.rules = rules;
                self.rule_engine = Arc::new(RuleEngine::new(
                    self.rules.clone(),
                    self.rule_groups.clone(),
                    None,
        ...
        while let Some(msg) = self.receiver.recv().await {
            match msg {
                RuleStoreMessage::SetRules(rules) => {
                    ...
                    if let Some(store) = &self.store {
                        let rules_to_save: Vec<(String, serde_json::Value)> = self
                            .rules
                            .iter()
